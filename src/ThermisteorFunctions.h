...
void spiSetup2(){
	for(int i=0; i<20; i++){
		pinMode(chipSelector(i), OUTPUT);
		digitalWrite(chipSelector(i), LOW);
	}
	pinMode(clk, OUTPUT);
	pinMode(mosi, OUTPUT);
	pinMode(miso, INPUT);
...
void readAll(){
	...
	for(int chip = 0; chip<20; chip++){
		for(int mux = 0; mux < 8; mux++){
			temps[chip][mux] = getTemp(readVoltage2(chipSelector(chip), mux));
		}
	}
	SPI.endTransaction();
}
...
float readVoltage2(int CSpin, uint8_t mux){
	int T = 40;
	uint16_t data = 0;
	digitalWrite(CSpin, HIGH);
	delayMicroseconds(T);
	...
	//D2
	digitalWrite(clk, LOW);digitalWrite(mosi, (mux&0b00000100)>0);
	delayMicroseconds(T);
	digitalWrite(clk, HIGH);
	delayMicroseconds(T);

	//D1
	digitalWrite(clk, LOW);digitalWrite(mosi, (mux&0b00000010)>0);
	delayMicroseconds(T);
	digitalWrite(clk, HIGH);
	delayMicroseconds(T);

	//D0
	digitalWrite(clk, LOW);digitalWrite(mosi, (mux&0b00000001)>0);
	delayMicroseconds(T);
	digitalWrite(clk, HIGH);
	delayMicroseconds(T);

	//empty bit
	digitalWrite(clk, LOW);//digitalWrite(mosi, LOW);
	delayMicroseconds(T);
	digitalWrite(clk, HIGH);
	delayMicroseconds(T);
